...
wmlmm <- function(formula, data = NULL, ID3, ID2, ID1, wj, wij, type, rot = NULL){
  ...
  s <- length(DELTA)
  h_matrix = diag(s)[1,] #aqui é um vetor
  ...
  dp_u <- sqrt(as.numeric(variances_resids$var_u))
  ...
  list_genlin <-list(beta = list(coefficients = beta, standard_errors = dp_beta, initial_values = beta0),
                     ...
                     num_time_obs = tt, call = clprint, fitted_values = as.numeric(variances_resids$yhat),
                     individual_residuals = as.numeric(variances_resids$v),
                     group_residuals = list(coefficients = as.numeric(variances_resids$u), standard_errors = dp_u),
                     iterations = itera-1
  )
